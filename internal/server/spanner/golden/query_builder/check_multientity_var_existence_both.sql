		...
		slot1 AS (
			...
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk') AND t.entity1 IN ('geoId/01001','geoId/02013')
		),
		slot2 AS (
			SELECT DISTINCT t.variable_measured, t.entity2 AS entity
			FROM TimeSeries@{FORCE_INDEX=TimeSeriesByEntity2} AS t
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk') AND t.entity2 IN ('geoId/01001','geoId/02013') AND t.entity2 IS NOT NULL
		),
		slot3 AS (
			SELECT DISTINCT t.variable_measured, t.entity3 AS entity
			FROM TimeSeries@{FORCE_INDEX=TimeSeriesByEntity3} AS t
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk') AND t.entity3 IN ('geoId/01001','geoId/02013') AND t.entity3 IS NOT NULL AND t.entity2 IS NOT NULL
		)
