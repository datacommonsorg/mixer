		...
		slot1 AS (
			...
			WHERE t.variable_measured = 'AirPollutant_Cancer_Risk'
		),
		slot2 AS (
			SELECT DISTINCT t.variable_measured, t.entity2 AS entity
			FROM TimeSeries@{FORCE_INDEX=TimeSeriesByEntity2} AS t
			WHERE t.variable_measured = 'AirPollutant_Cancer_Risk' AND t.entity2 IS NOT NULL
		),
		slot3 AS (
			SELECT DISTINCT t.variable_measured, t.entity3 AS entity
			FROM TimeSeries@{FORCE_INDEX=TimeSeriesByEntity3} AS t
			WHERE t.variable_measured = 'AirPollutant_Cancer_Risk' AND t.entity3 IS NOT NULL AND t.entity2 IS NOT NULL
		)
