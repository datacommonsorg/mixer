		...
		slot1 AS (
			...
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk')
		),
		slot2 AS (
			...
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk') AND t.entity2 IS NOT NULL
		),
		slot3 AS (
			...
			WHERE t.variable_measured IN ('AirPollutant_Cancer_Risk') AND t.entity3 IS NOT NULL AND t.entity2 IS NOT NULL
		)
