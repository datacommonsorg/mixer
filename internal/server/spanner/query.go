...
func (sc *SpannerClient) GetObservations(ctx context.Context, variables []string, entities []string, filterObs bool) ([]*Observation, error) {
	...
	err := sc.queryAndCollect(
		ctx,
		buildBaseObsStatement(variables, entities, filterObs),
		func() interface{} {
			return &Observation{}
		},
		func(rowStruct interface{}) {
...
func (sc *SpannerClient) GetObservationsContainedInPlace(ctx context.Context, variables []string, containedInPlace *v2.ContainedInPlace, filterObs bool) ([]*Observation, error) {
	var observations []*Observation
	if len(variables) == 0 || containedInPlace == nil {
		return observations, nil
	}

	stmt := buildBaseObsStatement(variables, []string{} /*entities*/, filterObs)
