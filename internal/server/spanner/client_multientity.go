...
// newMultiEntityClient initializes the client with table configuration and the base client's query configuration.
func newMultiEntityClient(client SpannerClient, tableConfig TableConfig) (*multiEntityClient, error) {
	sc, ok := client.(*spannerDatabaseClient)
	if !ok {
		return nil, fmt.Errorf("newMultiEntityClient: expected *spannerDatabaseClient, got %T", client)
	}
	queryBuilder, err := NewMultiEntityQueryBuilder(tableConfig, sc.queryConfig)
	if err != nil {
